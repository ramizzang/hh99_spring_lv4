...
public class CommentService {
    ...
    @Transactional
    public Long  deleteComment(Long commentId, User user) throws IllegalAccessException {
        // 존재하는 댓글인지 확인, 댓글에 있는 강의 정보 가져온다
        Comment comment = findCommentById(commentId);
        // 본인 확인
        checkUserAuthorization(comment.getUser(), user);
    ...
    private Comment findCommentById(Long commentId){
        return commentRepository.findById(commentId).orElseThrow(
                ()->new IllegalArgumentException("삭제된 댓글 입니다.")
        );
    }

    private void checkUserAuthorization(User commentUser, User requestUser) throws IllegalAccessException {
        if (!commentUser.getId().equals(requestUser.getId())) {
            throw new IllegalAccessException("댓글에 대한 권한이 없습니다.");
        }
    }
